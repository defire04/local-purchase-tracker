...
function render() {
  ...
  if (!list.length) {
    el.innerHTML = `<div class="empty-state"><div class="empty-ico">🔍</div><h3>Ничего не найдено</h3><p>Попробуйте изменить фильтры или добавьте первый товар</p></div>`;
    return;
  }

  const cols = [
    { id: 'name',     label: 'Название',        sortable: true,  cls: 'col-name'     },
    { id: 'brand',    label: 'Бренд',           sortable: true,  cls: 'col-brand'    },
    { id: 'shop',     label: 'Магазин / Заказ', sortable: false, cls: 'col-shop'     },
    { id: 'date',     label: 'Дата',            sortable: true,  cls: 'col-date'     },
    { id: 'price',    label: 'Цена',            sortable: true,  cls: 'col-price'    },
    { id: 'warranty', label: 'Гарантия',        sortable: true,  cls: 'col-warranty' },
    { id: 'status',   label: 'Статус',          sortable: false, cls: 'col-status'   },
  ];
...
function renderGroupedRows(list) {
  ...
  groups.forEach(g => {
    ...
    // Shop color for order/shop grouping
    ...
    // Header inner content by groupBy type
    let headerInner = '';
    if (groupBy === 'order') {
      const firstName = g.items[0]?.name || '';
      const namePart  = g.items.length === 1
        ? `<span class="g-name">${esc(firstName)}</span>`
        : `<span class="g-name">${esc(firstName)} <span class="g-more">+${g.items.length - 1} ещё</span></span>`;
      ...
      headerInner = `${shopBadge(shopId)}${orderPart}${namePart}
        ...
        ${g.items.length > 1 ? `<span class="g-count">${g.items.length} поз.</span>` : ''}`;
      ...
      headerInner = `${shopBadge(shopId)}${namePart}
        <span class="g-count">${g.items.length} поз.</span>`;

    } else if (groupBy === 'month') {
      const [y, m]   = g.key.split('-');
      const monthLbl = g.key === '—' ? '—' : new Date(+y, +m - 1, 1).toLocaleDateString('ru-RU', { month: 'long', year: 'numeric' });
...
function renderDetail(it) {
  ...
  if (!isSvc && it.serialNumber)   specsHtml += specRow('Серийный номер', `<code>${esc(it.serialNumber)}</code>`);
  if (!isSvc && it.warrantyMonths) specsHtml += specRow('Гарантия', `${it.warrantyMonths} мес. → ${wEnd ? fmtDate(wEnd) : '—'}`);
  if (isSvc  && it.executor)       specsHtml += specRow('Исполнитель', esc(it.executor));
  specs.forEach(([k, v]) => specsHtml += specRow(k, esc(v)));
  specsHtml += '</div>';
  if (it.note) specsHtml += `<div class="detail-note">${esc(it.note)}</div>`;

  // Right column — all buttons vertical, same width
  let rightHtml = '';

  // Documents section: receipts + shop/ek links, all as full-width column buttons
  const docLinks = [];
  rcpts.forEach(r => {
    const icon  = r.type === 'url' ? '🔗' : r.type === 'pdf' ? '📄' : '📷';
    const label = r.label || (r.type === 'url' ? 'Ссылка' : r.type === 'pdf' ? 'PDF' : 'Фото');
  ...
  if (it.link)             docLinks.push(`<a class="btn btn-ghost btn-sm btn-block" href="${esc(it.link)}"   target="_blank">🏪 Магазин</a>`);
  if (it.ekLink && !isSvc) docLinks.push(`<a class="btn btn-ghost btn-sm btn-block" href="${esc(it.ekLink)}" target="_blank">📊 ek.ua</a>`);

  if (docLinks.length) {
    rightHtml += `<div class="dr-section">
      <div class="dr-title">Документы и ссылки</div>
  ...
  if (evs.length) {
    rightHtml += `<div class="dr-section">
      <div class="dr-title">История</div>
      ${evs.map(ev => `<div class="ev-compact">
        <div class="ev-compact-header">
          <span class="ev-date">${esc(ev.date || '')}</span>
          <span class="ev-type-label ev-type-${ev.type}">${esc(EV_LABELS[ev.type] || ev.type)}</span>
        </div>
  ...
  // Action buttons — vertical, full width, separated by border-top
  rightHtml += `<div class="detail-actions">
    <button class="btn btn-secondary btn-sm btn-block" onclick="openEdit('${it.id}');event.stopPropagation()">✏️ Редактировать</button>
    <button class="btn btn-ghost btn-sm btn-block"     onclick="openAddEvent('${it.id}');event.stopPropagation()">⚡ Событие</button>
    <button class="btn btn-danger btn-sm btn-block"    onclick="deleteItem('${it.id}');event.stopPropagation()">🗑 Удалить</button>
  </div>`;
  ...
  const rows = shops.map(s => {
    ...
    return `<div class="settings-list-item">
      ...
      <div class="sli-actions">
        <button class="btn btn-ghost btn-sm btn-icon" onclick="openEditShop('${esc(s.id)}')" title="Редактировать">✏️</button>
        <button class="btn btn-danger btn-sm btn-icon" onclick="deleteShop('${esc(s.id)}')" title="Удалить">🗑</button>
      </div>
    </div>`;
  }).join('');

  return `<div class="settings-card">
    <div class="settings-card-hd"><h3>🏪 Магазины</h3><button class="btn btn-primary btn-sm" onclick="openAddShop()">+ Добавить</button></div>
    <div class="settings-list">${rows || `<div style="padding:1rem 1.1rem">
      <div style="color:var(--yellow);font-size:.88rem;font-weight:600;margin-bottom:.35rem">⚠ Магазины не загружены</div>
      <div style="color:var(--text3);font-size:.82rem;line-height:1.6">
        <b style="color:var(--text2)">Chrome:</b> откройте папку <code>purchases/</code><br>
        <b style="color:var(--text2)">Firefox:</b> выберите все три JSON-файла одновременно
      </div></div>`}</div>
  </div>`;
}
...
function renderCatsCard() {
  const rows = cats.map(c => `<div class="settings-list-item">
    <div class="sli-info">
      <div class="sli-name">${esc(c.name)} ${c.isService ? '<span class="tag" style="font-size:.7rem">Услуга</span>' : ''}</div>
      <div class="sli-sub">${c.id}</div>
    </div>
    <div class="sli-actions">
      <button class="btn btn-danger btn-sm btn-icon" onclick="deleteCat('${esc(c.id)}')" title="Удалить">🗑</button>
    </div>
  </div>`).join('');

  return `<div class="settings-card">
    <div class="settings-card-hd"><h3>🗂 Категории</h3><button class="btn btn-primary btn-sm" onclick="openAddCat()">+ Добавить</button></div>
    <div class="settings-list">${rows || '<div style="padding:1rem;color:var(--text3);font-size:.88rem">Список пуст</div>'}</div>
  </div>`;
}
